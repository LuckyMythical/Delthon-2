...
class Moderator(commands.Cog):
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        try:
            ...
            await ctx.send(default.actionmessage("kicked", delete_after=5))
    ...
    async def massban(self, ctx, reason: ActionReason, *members: MemberID):
        ...
        try:
            ...
            await ctx.send(default.actionmessage("massbanned", mass=True, delete_after=5))
        except Exception as e:
            await ctx.send(e)
    ...
    async def unban(self, ctx, member: MemberID, *, reason: str = None):
        ...
        try:
            ...
            await ctx.send(default.actionmessage("unbanned", delete_after=5))
        except Exception as e:
            await ctx.send(e, delete_after=5)
    ...
    async def mute(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        if not muted_role:
            return await ctx.send("Are you sure you've made a role called **Muted**? Remember that it's case sensetive too...", delete_after=5)

        try:
            await member.add_roles(muted_role, reason=default.responsible(ctx.author, reason))
            await ctx.send(default.actionmessage("muted", delete_after=5))
        except Exception as e:
            await ctx.send(e, delete_after=5)
    ...
    async def unmute(self, ctx, member: discord.Member, *, reason: str = None):
        ...
        try:
            ...
            await ctx.send(default.actionmessage("unmuted", delete_after=5))
        except Exception as e:
            await ctx.send(e)
    ...
    async def announcerole(self, ctx, *, role: discord.Role):
        ...
        if role == ctx.guild.default_role:
            return await ctx.send("To prevent abuse, I won't allow mentionable role for everyone/here role.", delete_after=5)

        if ctx.author.top_role.position <= role.position:
            return await ctx.send("It seems like the role you attempt to mention is over your permissions, therefor I won't allow you.", delete_after=5)

        if ctx.me.top_role.position <= role.position:
            return await ctx.send("This role is above my permissions, I can't make it mentionable ;-;", delete_after=5)
        ...
        while True:
            ...
            try:
                ...
                if checker.author.id == ctx.author.id:
                    ...
                    return await msg.edit(content=f"**{role.name}** mentioned by **{ctx.author}** in {checker.channel.mention}", delete_after=5)
                    break
                else:
                    await checker.delete()
            except asyncio.TimeoutError:
                await role.edit(mentionable=False, reason=f"[ {ctx.author} ] announcerole command")
                return await msg.edit(content=f"**{role.name}** was never mentioned by **{ctx.author}**...", delete_after=5)
                break
    ...
    async def do_removal(self, ctx, limit, predicate, *, before=None, after=None, message=True):
        if limit > 2000:
            return await ctx.send(f'Too many messages to search given ({limit}/2000)', delete_after=5)
        ...
        try:
            ...
            return await ctx.send('I do not have permissions to delete messages.', delete_after=5)
        except discord.HTTPException as e:
            return await ctx.send(f'Error: {e} (try a smaller search?)', delete_after=5)

        deleted = len(deleted)
        if message is True:
            await ctx.send(f'🚮 Successfully removed {deleted} message{"" if deleted == 1 else "s"}.', delete_after=5)
    ...
    async def contains(self, ctx, *, substr: str):
        ...
        """
        if len(substr) < 3:
            await ctx.send('The substring length must be at least 3 characters.', delete_after=5)
        else:
    ...
    async def _reactions(self, ctx, search=100):
        ...
        if search > 2000:
            return await ctx.send(f'Too many messages to search for ({search}/2000)', delete_after=5)
