...
class Discord_Info(commands.Cog):
        ...
        for user in ctx.guild.members:
            if ctx.channel.permissions_for(user).kick_members or \
               ctx.channel.permissions_for(user).ban_members:
                ...
                if not user.bot and user.status == discord.Status.idle:
                    idle.append(f"**{user}**")
                if not user.bot and user.status == discord.Status.dnd:
                    dnd.append(f"**{user}**")
                if not user.bot and user.status == discord.Status.offline:
    ...
    @commands.group(aliases=['serverinfo', 'serverstats'])
    @commands.guild_only()
    async def server(self, ctx):
        """ Check info about current server """
        if ctx.invoked_subcommand is None:
            find_bots = sum(1 for member in ctx.guild.members if member.bot)
            find_humans = ctx.guild.member_count - find_bots
            ...
            embed.add_field(name="Humans", value=find_humans, inline=True)
    ...
    @server.command(name="icon", aliases=["logo"])
    ...
    async def user(self, ctx, *, user: discord.Member = None):
        ...
        if user.id in self.config.contestwinner:
          ack = "Contest Winner"
        ...
        embed.add_field(name="Acknowledgments", value=ack, inline=True)
